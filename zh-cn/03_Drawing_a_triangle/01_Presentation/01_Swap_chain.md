...
in [a future chapter](!zh-cn/Drawing_a_triangle/Swap_chain_recreation). For now we'll
...
Now run the application to ensure that the swap chain is created successfully! If at this point you get an access violation error in `vkCreateSwapchainKHR` or see a message like `Failed to find 'vkGetInstanceProcAddress' in layer SteamOverlayVulkanLayer.dll`, then see the [FAQ entry](!zh-cn/FAQ) about the Steam overlay layer.
